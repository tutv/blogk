...
if ( ! function_exists( 'gravit_posted_on' ) ) :
	...
	function gravit_posted_on() {
		$class_time_ago = '';
		if ( is_home() ) {
			$class_time_ago = 'time_ago';
		}
		$time_string = '<time class="entry-date published ' . $class_time_ago . '" datetime="%1$s" title="%1$s">%2$s</time>';
		...
		if ( comments_open() ) :
			...
			echo comments_popup_link( '<span class="leave-reply">' . __( 'Bình luận', 'gravit' ) . '</span>', __( '1 bình luận', 'gravit' ), __( '% bình luận', 'gravit' ) ) . '</span>';
		endif; // comments_open();
